...
import { scanClipping, splitFrontmatter, bodyText } from "../src/core/scan";
import {
  COMBOLANDS_BODY,
  COMBOLANDS_FM,
  MANGA_BODY,
...
describe("scanClipping properties", () => {
  ...
  it("gives a clipping no status it was not written with", () => {
    expect(scanClipping("Clippings/a.md", {}, "").properties.status).toBeUndefined();
    expect(scanClipping("Clippings/a.md", { status: "read" }, "").properties.status).toEqual(["read"]);
  });
...
describe("bodyText", () => {
  it("keeps prose so a word from the article is findable", () => {
    expect(bodyText("The checkout flow uses Apple Pay.")).toBe(
      "the checkout flow uses apple pay."
    );
  });

  it("strips addresses, which would answer searches with CDN paths", () => {
    const out = bodyText("See https://cdn.example.com/design-system/hero.png for it");
    expect(out).not.toContain("cdn.example.com");
    expect(out).not.toContain("design-system");
    expect(out).toContain("see");
    expect(out).toContain("for it");
  });

  it("keeps a link's text and drops its target", () => {
    expect(bodyText("[A checkout page](https://example.test/payments)")).toBe("a checkout page");
    expect(bodyText("![a dark dashboard](https://x.test/a.png)")).toBe("a dark dashboard");
  });

  it("keeps a wikilink's label, or its target when unlabelled", () => {
    expect(bodyText("![[Attachments/a.png|hero shot]]")).toBe("hero shot");
    expect(bodyText("[[Some Note]]")).toBe("some note");
  });

  it("drops html and markdown furniture", () => {
    expect(bodyText("## Heading\n\n> **bold** quote")).toBe("heading bold quote");
    expect(bodyText('<img src="a.png" alt="x"> after')).toBe("after");
  });

  it("leaves fenced code out, so snippets do not answer prose searches", () => {
    expect(bodyText("before\n```\nconst checkout = 1;\n```\nafter")).toBe("before after");
  });

  it("caps the body, so one longform clipping cannot dominate memory", () => {
    expect(bodyText("word ".repeat(20000)).length).toBeLessThanOrEqual(20000);
  });

  it("indexes the whole of what the plugin writes into a body", () => {
    // A PDF's text and an article's are chosen content the whole way down,
    // and both are bounded well inside this. A word two thirds of the way
    // through a document is a word you will search for.
    const long = `${"word ".repeat(2400)}pelican`;
    expect(bodyText(long)).toContain("pelican");
  });
});

describe("scanClipping body indexing", () => {
  const body = [
    "---",
    ...
    "---",
    "",
    "Stripe hides the Apple Pay button until the sheet opens.",
  ].join("\n");

  it("puts the article's own words in the haystack", () => {
    const r = scanClipping("Clippings/a.md", { title: "Checkout teardown" }, body);
    expect(r.haystack).toContain("apple pay");
  });

  it("still leads with the title, so a title hit outranks a body hit", () => {
    const r = scanClipping("Clippings/a.md", { title: "Checkout teardown" }, body);
    expect(r.haystack.indexOf("checkout teardown")).toBeLessThan(r.haystack.indexOf("apple pay"));
  });
});

describe("note and summary in the haystack", () => {
  it("finds a word from the reader's note, which lives in frontmatter not the body", () => {
    const r = scanClipping("Clippings/a.md", { title: "A", note: "saved for the checkout layout" }, "---\ntitle: A\n---\n");
    expect(r.haystack).toContain("checkout layout");
  });

  it("finds a word from the model's summary", () => {
    const r = scanClipping("Clippings/a.md", { title: "A", summary: "Dark dashboard with a sidebar" }, "");
    expect(r.haystack).toContain("dark dashboard");
  });

  it("puts them ahead of the body, so they outrank an article's prose", () => {
    const body = "---\ntitle: A\n---\nlong article text mentioning sidebar far down";
    const r = scanClipping("Clippings/a.md", { title: "A", summary: "sidebar" }, body);
    expect(r.haystack.indexOf("sidebar")).toBeLessThan(r.haystack.indexOf("long article"));
  });
});
