...
describe("showsPosterAgain", () => {
  const video = (over: Partial<{ poster: string; hasSource: boolean; played: boolean }> = {}) => ({
    poster: "app://vault/frame.jpg",
    hasSource: true,
    played: true,
    ...over,
  });

  it("puts the cover back on a clip that has played", () => {
    expect(showsPosterAgain(video())).toBe(true);
  });

  it("leaves a clip that never started alone", () => {
    // Nothing is covering the poster, and load() on every tile that happens
    // not to be playing is what apply() would do sixty times a second.
    expect(showsPosterAgain(video({ played: false }))).toBe(false);
  });

  it("leaves a clip with no poster showing its last frame", () => {
    // Reloading raises the show-poster flag over nothing at all, and the tile
    // goes blank. The frame it stopped on is the best still it has.
    expect(showsPosterAgain(video({ poster: "" }))).toBe(false);
  });

  it("does nothing for an element that has no source yet", () => {
    // Off-screen tiles keep their src in a data attribute until they near the
    // viewport; load() there would run the selection algorithm over nothing.
    expect(showsPosterAgain(video({ hasSource: false }))).toBe(false);
  });
});
