...
import { buildNote, buildPastedImageNote, buildScanNote } from "../src/core/resolve";
import { splitFrontmatter } from "../src/core/scan";
...
describe("the article a page turned out to have", () => {
  const article = "## The heading\n\nA paragraph of the piece itself.";

  it("is kept under the address, with a rule between", () => {
    const note = buildNote(link({ article }));
    const at = note.indexOf(article);
    expect(at).toBeGreaterThan(note.indexOf("https://x.com/someone"));
    expect(note).toContain("\n\n---\n\n## The heading");
  });

  it("does not disturb the frontmatter, though it carries a rule of its own", () => {
    // A second --- in the body would be frontmatter's closing line if
    // anything read it loosely. The reader takes the first one after line 0,
    // which is the real one.
    const note = buildNote(link({ article }));
    const { yaml, rest } = splitFrontmatter(note);
    expect(yaml).toContain("title:");
    expect(yaml).not.toContain("The heading");
    expect(rest).toContain("The heading");
  });

  it("reaches the search index, which is half the reason for keeping it", () => {
    const note = buildNote(link({ article: "Ochre and cadmium in Bauhaus posters" }));
    expect(bodyText(splitFrontmatter(note).rest)).toContain("cadmium");
  });

  it("changes nothing for a link that is not a piece of writing", () => {
    expect(buildNote(link())).toBe(buildNote(link({ article: "" })));
  });

  it("goes into a scanned page's note too, under its picture", () => {
    const note = buildScanNote("T", "https://e.com/a", "Attachments/scan.png", "2026-09-16", "", article);
    expect(note).toContain("![[Attachments/scan.png]]");
    expect(note).toContain("## The heading");
    expect(buildScanNote("T", "https://e.com/a", "Attachments/scan.png", "2026-09-16")).not.toContain(
      "---\n\n##"
    );
  });
});
