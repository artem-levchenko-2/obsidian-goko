...
describe("matching text the reader never sees", () => {
  ...
  const unrelated =
    "MILD • •••• WILD ADV WASHED CO-FERMENT NET WT 13 OZ (369 G) OF SAMPLE BLEND " +
  ...
  const real = "9:41 Good Afternoon 10 coffees on shelf tropical weather ONYX Sample Roasters";

  const items = [
    { label: "Bag of coffee", text: unrelated },
    { label: "Post by @someone on X", text: real },
  ];
