...
describe("grid facet", () => {
  const registry = { grids: new Set(["Stripe", "Inspiration"]), home: "Inbox" };
  const defs = facetDefs([], registry);
  const gridDef = defs.find((d) => d.id === "grid")!;

  const filed = { ...tile("f"), record: { ...tile("f").record, grid: "Stripe" } };
  const unfiled = tile("u");
  const orphan = { ...tile("o"), record: { ...tile("o").record, grid: "Gone" } };

  it("is only offered when told what the grids are", () => {
    expect(facetDefs([]).some((d) => d.id === "grid")).toBe(false);
    expect(gridDef).toBeDefined();
  });

  it("reports a filed card by its grid", () => {
    expect(gridValueOf(filed.record, gridDef)).toBe("Stripe");
  });

  it("reports an unfiled card as home", () => {
    expect(gridValueOf(unfiled.record, gridDef)).toBe("Inbox");
  });

  it("names the grid an orphan still points at, marked as missing", () => {
    expect(gridValueOf(orphan.record, gridDef)).toBe(`${MISSING_GRID_PREFIX}Gone`);
  });

  it("counts the orphans as their own row, so the silent state is visible", () => {
    const counts = facetsOf([filed, unfiled, orphan], defs).grid;
    expect(counts).toEqual(
      expect.arrayContaining([
        { value: "Stripe", count: 1 },
        { value: "Inbox", count: 1 },
        { value: `${MISSING_GRID_PREFIX}Gone`, count: 1 },
      ])
    );
  });

  it("filters to the orphans alone", () => {
    const filter: FilterState = { grid: [`${MISSING_GRID_PREFIX}Gone`] };
    expect(matchesFilter(orphan, filter, defs)).toBe(true);
    expect(matchesFilter(filed, filter, defs)).toBe(false);
    expect(matchesFilter(unfiled, filter, defs)).toBe(false);
  });

  it("lets a smart view select by grid, which nothing could before", () => {
    expect(smartMembers([filed, unfiled, orphan], { grid: ["Stripe"] }, defs).map((t) => t.id)).toEqual(["f"]);
  });
});
