...
import { extractPageImage, knownHostThumbnail, needsPageCover, decodeEntities } from "../src/core/page-cover";
...
describe("decodeEntities", () => {
  it("decodes hex and decimal numeric entities, which Threads and blogs both emit", () => {
    expect(decodeEntities("&#xac00;&#xb098;&#xb2e4; (&#064;someone)")).toBe("가나다 (@someone)");
    expect(decodeEntities("Chef Bunny&#8217;s version")).toBe("Chef Bunny’s version");
  });

  it("decodes the named entities pages use in titles", () => {
    expect(decodeEntities("A &amp; B &mdash; C&hellip; &laquo;D&raquo;")).toBe("A & B — C… «D»");
    expect(decodeEntities("&quot;q&quot; &lt;t&gt; &#39;s&apos;")).toBe('"q" <t> \'s\'');
  });

  it("leaves an entity it does not know as written", () => {
    expect(decodeEntities("x &zzzz; y")).toBe("x &zzzz; y");
  });

  it("unwraps a numeric entity that was escaped twice, which Threads does to its own title", () => {
    // Strictly `&amp;#064;` is the five characters `&#064;`, and this used to
    // say so. But Threads escapes its og:title twice, and the cost was not
    // only an ugly title: refineThreadsLink looks for a literal @ to find the
    // handle, so the post kept the generic title as well.
    expect(decodeEntities("Alex (&amp;#064;someone__) &amp;#x443; &amp;#x434;")).toBe(
      "Alex (@someone__) у д"
    );
  });

  it("unwraps numeric only, so markup cannot arrive in named form", () => {
    expect(decodeEntities("&amp;lt;script&amp;gt;")).toBe("&lt;script&gt;");
  });

  it("unwraps one layer only, so a third escape stays text", () => {
    expect(decodeEntities("&amp;amp;#39;")).toBe("&amp;#39;");
  });

  it("turns an impossible code point into the replacement character", () => {
