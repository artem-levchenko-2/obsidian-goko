...
describe("shuffle rows", () => {
  it("offers Shuffle on an ordered wall, and nothing to restore", () => {
    expect(find(context(), "view:shuffle")?.label).toBe("Shuffle");
    expect(find(context(), "view:newest")).toBeUndefined();
  });

  it("offers Shuffle again and Newest first on a shuffled wall", () => {
    const ctx = context({ order: "shuffled" });
    expect(find(ctx, "view:shuffle")?.label).toBe("Shuffle again");
    expect(find(ctx, "view:newest")).toBeDefined();
  });

  it("keeps the palette open on shuffle, so it can be done a few times", () => {
    expect(find(context(), "view:shuffle")?.keepOpen).toBe(true);
  });
});

describe("file as suggested", () => {
  it("is offered for a selection with suggestions, counted", () => {
    expect(find(context({ selection: ["a"], suggested: 1 }), "selection:file-suggested")?.label).toBe(
      "File as suggested"
    );
    expect(
      find(context({ selection: ["a", "b", "c"], suggested: 2 }), "selection:file-suggested")?.label
    ).toBe("File 2 as suggested");
  });

  it("is absent when nothing selected has one", () => {
    expect(find(context({ selection: ["a"] }), "selection:file-suggested")).toBeUndefined();
    expect(find(context({ selection: ["a"], suggested: 0 }), "selection:file-suggested")).toBeUndefined();
  });
});

describe("the group row", () => {
  it("is offered on the inbox, worded for the state it would change", () => {
    expect(find(context({ canGroup: true }), "view:group")?.label).toBe("Group inbox by suggestion");
    expect(find(context({ canGroup: true, grouped: true }), "view:group")?.label).toBe("Ungroup inbox");
  });

  it("is absent anywhere but the inbox", () => {
    expect(find(context(), "view:group")).toBeUndefined();
    expect(find(context({ canGroup: false }), "view:group")).toBeUndefined();
  });

  it("runs the toggle rather than only naming it", () => {
    let ran = 0;
    const ctx = context({ canGroup: true, actions: { ...actions, groupInbox: () => ran++ } });
    find(ctx, "view:group")?.run?.();
    expect(ran).toBe(1);
  });
});

describe("order rows", () => {
  it("offers Recently updated on a wall that is not in it", () => {
    expect(find(context(), "view:updated")).toBeDefined();
    expect(find(context({ order: "shuffled" }), "view:updated")).toBeDefined();
  });

  it("hides each order the wall is already in, since it would change nothing", () => {
    const ctx = context({ order: "updated" });
    expect(find(ctx, "view:updated")).toBeUndefined();
    expect(find(ctx, "view:newest")).toBeDefined();
    expect(find(ctx, "view:shuffle")?.label).toBe("Shuffle");
  });

  it("runs the action rather than only naming it", () => {
    let ran = 0;
    const ctx = context({ actions: { ...actions, recentlyUpdated: () => ran++ } });
    find(ctx, "view:updated")?.run?.();
    expect(ran).toBe(1);
  });
});

describe("AI rows", () => {
  it("offers Describe with AI for a selection, worded for its size", () => {
    expect(find(context({ selection: ["a"] }), "selection:describe")?.label).toBe("Describe with AI");
    expect(find(context({ selection: ["a", "b", "c"] }), "selection:describe")?.label).toBe("Describe 3 with AI");
  });

  it("does not offer it with nothing selected", () => {
    expect(find(context(), "selection:describe")).toBeUndefined();
  });

  it("always offers the bulk row", () => {
    expect(find(context(), "capture:describe-all")).toBeDefined();
  });
});
