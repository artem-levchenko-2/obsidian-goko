...
export interface SheetRow {
  ...
  detailIcon?: string;
  /**
   * A colour for the row's icon, for a screen whose rows *are* colours. The
   * icon picker's swatches say what they are by their shape; a colour picker's
   * cannot, and nine identical grey circles is not a choice.
   */
  tint?: string;
...
export class Sheet {
  ...
  private paintSwatch(host: HTMLElement, row: SheetRow): HTMLElement {
    ...
    if (row.tint) el.style.color = row.tint;
    el.onclick = (event: MouseEvent) => {
      event.stopPropagation();
      this.choose(row);
    };
