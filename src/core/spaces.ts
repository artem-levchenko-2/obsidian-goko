 ...
 */
/**
 * The colours a grid can be given, named rather than written as hex.
 *
 * Obsidian ships each of these as a theme variable, so a named colour follows
 * the theme the vault is wearing and stays legible in both — a hex picked
 * against a dark theme is a hex that disappears in a light one. "" is no
 * colour, which is what every grid has until one is chosen.
 */
...
/** Whether a stored value is a colour this build knows how to paint. */
export function isGridColor(value: unknown): value is GridColor {
  return typeof value === "string" && (GRID_COLORS as readonly string[]).includes(value);
}

/** The CSS variable a colour paints with, or "" for a grid with none. */
export function gridColorVar(color: string | undefined): string {
  return isGridColor(color) ? `var(--color-${color})` : "";
}

/**
 * The colour a grid is drawn in, whether or not anyone chose one.
 *
 * A colour is a label's second channel: you find "Design" among six chips by
 * its colour and confirm it by its name, which only works if every grid has
 * one. Most never will be given one by hand, and a wall of identical grey
 * chips makes the colour channel useless for all of them rather than for the
 * few that were skipped.
 *
 * So an unchosen colour is derived from the name instead: stable, because the
 * same name always hashes the same way, and distinct, because eight colours
 * over the handful of grids one person keeps rarely collide. A colour set by
 * hand always wins — this fills a gap, it does not overrule a choice.
 */
export function gridTint(grid: { name: string; color?: string }): string {
  if (isGridColor(grid.color)) return `var(--color-${grid.color})`;
  return `var(--color-${GRID_COLORS[nameHash(grid.name) % GRID_COLORS.length]})`;
}

/**
 * How the inbox is drawn wherever a grid would show its colour. Not a hue:
 * the inbox is where cards wait before they are filed, not a board, and it
 * reads as one of the rail's two plain rows with the library.
 */
export const HOME_TINT = "var(--text-normal)";

/** djb2, enough to scatter a dozen short names across eight buckets. */
...
/** How long a grid's line of description may be. A line, not a paragraph. */
export const MAX_GRID_DESCRIPTION = 120;
...
export interface GridSpace {
  ...
  icon: string;
  /**
   * A named colour, or absent for none. Carried here rather than in the
   * device's own settings because it is part of what a grid *is*: a board
   * that is orange on the desktop and grey on the phone is two boards.
   */
  color?: GridColor;
  /** One line saying what the grid is for, shown by its row in the rail. */
  description?: string;
  /**
