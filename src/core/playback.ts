...
/**
 * Whether stopping this video should put its cover back.
 *
 * A `<video>` shows its poster only until playback first starts. After that
 * the element holds whatever frame it is on, and `pause()` does not undo
 * that — the poster is governed by an internal flag the spec raises on
 * `load()` and lowers when play begins. So the only way back to the cover is
 * to load the element again.
 *
 * Which is expensive to get wrong, because apply() calls stop() on every
 * element that is not playing, on every frame of a scroll. Hence three
 * conditions rather than one:
 *
 * - it has played, or there is no frame covering the poster to clear;
 * - it has a source, or load() would run the selection algorithm over
 *   nothing;
 * - it has a poster, and this is the one that bites — reloading a video with
 *   no poster raises the flag over nothing at all and leaves the tile blank,
 *   which is worse than the last frame it was showing.
 */
export function showsPosterAgain(video: {
  poster: string;
  hasSource: boolean;
  played: boolean;
}): boolean {
  return video.played && video.hasSource && video.poster !== "";
}
...
export class PlaybackController {
  ...
  constructor(
    ...
    this.root.doc.addEventListener("visibilitychange", this.onVisibility);
  }
  ...
  private onVisibility = (): void => {
    if (this.root.doc.hidden) this.stopAll();
    else this.schedule();
  };
  ...
  private playHovered(): void {
    ...
    // From the beginning, every time. Pausing where it stopped and carrying
    // on from there is what a player does, and this is not a player: the
    // pointer arriving is a request to see the clip, and a clip that resumes
    // three seconds in has answered a question nobody asked. It also made the
    // wall feel stateful — the same card behaved differently depending on how
    // long you had hovered it an hour ago.
    for (const element of this.hovered) this.play(element, true);
  }
  ...
  private play(element: Playable, fromStart = false): void {
    if (element instanceof HTMLVideoElement) {
      // Guarded: rewinding a video that is already at the start makes the
      // browser reload the first frame, which shows as a flicker on a wall
      // where apply() runs on every scroll.
      if (fromStart && element.currentTime > 0) element.currentTime = 0;
      if (element.paused) void element.play().catch(() => undefined);
      // Recorded rather than inferred from currentTime, which is 0 both
      // before a clip starts and a frame after it does. This is the spec's
      // own show-poster flag, kept where we can read it.
    ...
    if (element instanceof HTMLVideoElement) {
      ...
      if (
        showsPosterAgain({
          ...
          played: Boolean(element.dataset.played),
        })
      ) {
        // Cleared first, so this is a transition rather than something that
        // runs again on every frame apply() spends deciding this element is
        // still not playing.
        delete element.dataset.played;
        // Stop, not pause: back to nought and back to the cover, which for a
        // card whose cover was chosen by hand is the whole point of having
        // chosen one.
        element.load();
      }
      return;
  ...
  destroy(): void {
    this.root.doc.removeEventListener("visibilitychange", this.onVisibility);
