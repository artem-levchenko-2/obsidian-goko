...
export type FacetSource = "property" | "kind" | "domain" | "grid" | "folder";
...
export interface FacetDef {
  ...
  now?: number;
  /**
   * The grid registry, for the grid facet: which names exist, and what home is
   * called. Carried here for the same reason `now` is — valuesFor sees one
   * tile and nothing else. Read only.
   */
  grids?: ReadonlySet<string>;
  home?: string;
  /** The folders of the grid on screen, for the folder facet. Read only. */
...
const GRID_FACET: FacetDef = {
  id: "grid",
  label: "Grid",
  icon: "layout-grid",
  keywords: "grid wall board filed where missing orphan narrow",
  source: "grid",
};

const FOLDER_FACET: FacetDef = {
  id: "folder",
  label: "Folder",
  icon: "folder",
  keywords: "folder pile stack inside loose narrow",
  source: "folder",
};

/**
 * How the folder facet spells a card filed in no folder, and one whose folder
 * no longer exists.
 *
 * Loose is a real answer rather than an absence — "the cards on this grid that
 * are not in any of its piles" is a question worth asking — so it gets a name
 * of its own rather than the empty-value row every other facet uses.
 */
export const LOOSE_FOLDER = "Loose";
export const MISSING_FOLDER_PREFIX = "Missing folder: ";

/**
 * How the grid facet spells a card whose `grid:` names nothing registered.
 *
 * spaces.ts sends such a card home on purpose, so deleting a grid never loses
 * a note. The cost of that choice is silence: the card is in the inbox with
 * nothing to say it was ever filed. This row is where it says so. A prefix
 * rather than the bare name, since the bare name reads as a grid that exists.
 */
export const MISSING_GRID_PREFIX = "Missing grid: ";
 ...
 */
/**
 * Which folder row a card belongs on.
 *
 * The registry carried here is the folders of the grid on screen: a folder
 * belongs to one grid, so a card whose key names a folder from another grid
 * reads as missing rather than as filed. Same bargain as the grid facet —
 * folders.ts shows such a card loose, and this is where it says so.
 */
export function folderValueOf(record: { folder: string }, def: FacetDef): string {
  const named = record.folder.trim();
  if (!named) return LOOSE_FOLDER;
  return def.folders?.has(named) ? named : `${MISSING_FOLDER_PREFIX}${named}`;
}

export function facetDefs(
  properties: string[],
  registry?: { grids: ReadonlySet<string>; home: string; folders?: ReadonlySet<string> }
): FacetDef[] {
  ...
  // The grid facet only when told what the grids are: without the registry
  // every filed card would read as missing.
  const grid = registry ? [{ ...GRID_FACET, grids: registry.grids, home: registry.home }] : [];
  // And the folder facet only when the grid on screen has folders at all:
  // a facet whose only row is Loose says nothing about anything.
  const folders =
    registry?.folders && registry.folders.size > 0
      ? [{ ...FOLDER_FACET, folders: registry.folders }]
      : [];
  return [...defs, KIND_FACET, DOMAIN_FACET, ...grid, ...folders];
}

/** Where a card is filed, as the grid facet reports it. Mirrors spaces.effectiveGrid
    without importing it: spaces imports this module, and a cycle is worse than
    three repeated lines. */
export function gridValueOf(record: { grid: string }, def: FacetDef): string {
  const named = record.grid.trim();
  const home = def.home ?? "";
  if (!named || named === home) return home;
  return def.grids?.has(named) ? named : `${MISSING_GRID_PREFIX}${named}`;
}
...
function valuesFor(tile: TileModel, def: FacetDef): string[] {
  switch (def.source) {
    ...
    case "grid":
      return [gridValueOf(tile.record, def)];
    case "folder":
      return [folderValueOf(tile.record, def)];
  }
}
