...
/**
 * Whether there is a filesystem to reveal a file in and a Downloads folder
 * to put one in.
 *
 * Asking require("fs") for it is not enough: Obsidian mobile leaves a
 * module shim in place that answers for some names, and the wall was
 * offering Reveal in Finder and Export to Downloads on a phone, where
 * neither can do anything. process.versions.electron is the honest
 * question — it is only ever there in the desktop app.
 */
export function systemAvailable(): boolean {
  const versions = (window as unknown as { process?: { versions?: { electron?: string } } })
    .process?.versions;
  return Boolean(versions?.electron) && nodeRequire("fs") !== null;
}
