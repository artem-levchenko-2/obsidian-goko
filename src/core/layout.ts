...
/**
 * A panel centred over a point and resting above it, slid sideways as far as
 * it has to be to stay on screen. Not placeMenu, whose flip is for a panel
 * hanging from its corner: a centred one that runs over the right edge
 * wants moving back by the overflow, not by its whole width.
 */
export function placeAbove(
  ...
  return {
    x: Math.max(margin, Math.min(point.x - panel.width / 2, viewport.width - panel.width - margin)),
    y: Math.max(margin, point.y - panel.height),
  };
}
