...
/** Free text, addresses, or plugin plumbing. Never suggested, but the settings list
    still shows them, so a user who wants one can add it by hand. */
const RESERVED = new Set(["title", "description", "source", "origin", "cover", "grid", "media"]);
...
/**
 * The properties worth offering for the tile's tag slot.
 *
 * Split by what the values actually look like rather than by the key's name,
 * so a key holding dates is offered as one. The current choice is kept at
 * the head even when it no longer survives the survey, so a slot set months
 * ago still shows what it is set to instead of reading as unset.
 */
