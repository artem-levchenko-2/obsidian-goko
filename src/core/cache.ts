...
export class MediaCache {
  ...
  mergeOutcome(outcome: ArchiveOutcome): void {
    // A failure that was about the moment rather than about the file leaves
    // no trace, so the next pass finds the key untouched and tries again.
    // Guarded here rather than at each of the four callers, which is what
    // one of them forgetting would cost.
    if (outcome.transient && !outcome.file) return;
