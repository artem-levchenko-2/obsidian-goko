...
import type { NarrowTarget } from "./palette-query";
...
export interface PaletteRow {
  ...
  clipping?: string;
  /** Set when the row completes a half-typed `@` or `#` token. */
  narrowTo?: NarrowTarget;
  /** A colour for the row's icon, for a row that stands for a coloured thing. */
  tint?: string;
}
...
export const NARROW_SECTION = "Narrow to";

const SECTION_ORDER = [NARROW_SECTION, "Actions", "Grids", "Filters", "Capture", CLIPPINGS_SECTION];
